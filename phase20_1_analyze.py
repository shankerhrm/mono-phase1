...
def analyze_tuning():
    with open('phase20_1_sweep/tuning_results.json', 'r') as f:
        results = json.load(f)
    ...
    # Group by restore_mult and target
    groups = {}
    for res in results:
        if 'error' in res:
            continue
        key = (res['restore_mult'], res['target'])
    ...
    print("\nPhase-20.1 Tuning Sweep Summary")
    print("restore_mult | target | p_extinct | mean_survival | mean_p_restore | variance | trade_events | food_ratio")
    print("-" * 110)
    for str_key, stats in sorted(summary.items()):
        rm = stats['restore_mult']
        target = stats['target']
        print(f"{rm:10.1f} | {target:6} | {stats['p_extinct']:8.2f} | {stats['mean_survival']:12.1f} | {stats['mean_p_restore']:13.3f} | {stats['variance']:8.4f} | {stats['trade_events']:11.2f} | {stats['food_ratio']:10.3f}")
    
    # Save summary
    with open('phase20_1_sweep/tuning_summary.json', 'w') as f:
        json.dump(summary, f, indent=2)
    print("\nSummary saved to phase20_1_sweep/tuning_summary.json")
